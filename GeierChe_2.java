...
public class GeierChe_2 extends Player {
	...
	private int strategyDynamic(int pointCard) {
		...
		int enemyValue = enemyStrategy_1.get(index + 1);
		if (enemyValue == 15) {
			card = 1;
		} else {
			card = ++enemyValue;
	...
	private void getStrategy(int pointCardValue, int enemyCard) {
		/*
		 * in der ersten Runde wird die erste Array-list mit den Karten des Gegners zu den
		 * jeweiligen PunkteKarten hintereinander gefüllt
		 */
		if (countGames == 1) {
		...
		/*
		 * in der zweiten Runde wird die zweite Array-list mit den Karten des Gegners zu den
		 * jeweiligen PunkteKarten hintereinander gefüllt
		 */
		else if (countGames == 2) {
